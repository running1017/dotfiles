...
display_help() {
    ...
    echo "  --base_packages    基本的なコマンドラインツールをインストールします (sudo必要)"
    echo "  --shell            zsh, eza, starshipのセットアップを行います (sudo必要)"
    echo "  --dotfiles         設定ファイルのシンボリックリンクを作成します (sudo必要)"
    echo "  --dev_guides       開発環境のセットアップガイドを表示します (sudo不要)"
    echo "  --ssh              SSH鍵を生成します (sudo必要)"
    echo "  --gui_tools        VSCodeとHackGenフォントをインストールします (sudo必要)"
    echo "  -h, --help         このヘルプを表示します (sudo不要)"
    echo ""
    echo "sudo権限について:"
    echo "  • sudo不要: --dev_guides, --help"
    echo "  • sudo必要: --base_packages, --shell, --dotfiles, --ssh, --gui_tools"
    echo ""
